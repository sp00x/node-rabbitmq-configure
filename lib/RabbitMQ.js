...
RabbitMQ.prototype.bindQueue = function (queueName, callback)
{
    ...
    {        
        ...
        Async.eachSeries(bindings,
            function bindNext(binding, next)
            {
                ...
                queueInfo.queue.bind(exchangeName, binding.topic, function (queue)
                {
                    ...
                    else
                    {
                        ...
                        log.error("Error binding queue '" + queueName + "' to exchange '" + exchangeName + "'");
                        return next();
                    }
                });
            },
            ...
            {
                if (errors > 0)
                    return me._error(callback, "Error binding queue '" + queueName + "' to exchange(s): " + failedExchanges.join(", "));
                else
                    return me._ok(callback);
...
RabbitMQ.prototype.subscribe = function (queueName, subscribeCallback, configuredCallback)
{
    ...
    }).addCallback(function(ok)
    {
        ...
        log.info(prefix + "Subscribed (" + ctag + ")");
        configuredCallback(null, function unsubscribe(callback)
        {
            try
            {
                var promise = q.queue.unsubscribe(ctag);
                return true;
            }
            catch (e)
            {
...
RabbitMQ.prototype.destroyExchange = function(exchangeName, options, callback)
{
    ...
    var en = this.expandExchangeName(exchangeName);
    var e = this.exchanges[en];
    if (e == null) return callback(null, false);

    e.exchange.destroy(options.ifUnused === true); // see: "If the exchange has queue bindings the server does not delete it but raises a channel exception instead."
    delete this.exchanges[en];
    return callback(null, true);
};

RabbitMQ.prototype.destroyQueue = function(queueName, options, callback)
{
    if (callback == null && typeof options == 'function')
    {
        callback = options;
    ...
    var qn = this.expandQueueName(queueName);
    var q = this.queues[qn];
    if (q == null) return callback(null, false);

    q.queue.destroy(options); // see: consumer_cancel_notify
    delete this.queues[qn];
    return callback(null, true);
};
