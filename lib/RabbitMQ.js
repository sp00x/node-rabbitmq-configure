...
RabbitMQ.prototype.__publish = function (me, exchangeName, topic, message, options, confirmCallback)
{
    ...
    me.exchanges[exchangeName].exchange.publish(topic, message, options, confirmCallback);
}

RabbitMQ.prototype.__connect = function(me, config, callback)
{
    var log = me.logger.contextualize("connect");
...
RabbitMQ.prototype.__declareExchange = function(me, exchangeName, exchangeConfig, callback)
{
    try
    {
        var log = me.logger.contextualize("declareExchange");
...
{
    var log = me.logger.contextualize("declareQueue");
    ...
    var q = me.connection.queue(declareQueueName, queueConfig, function (queue, dunno1, dunno2) // OR: q.on('queueDeclareOk', function(queue)
    {
        ...
        if (bindings != null && bindings instanceof Array && bindings.length > 0 && me.checkRole(bindings[0].roles))
        {
            ...
            log.info("Binding queue '" + queueName + "' to exchange '" + binding.exchange + "'..");
            q.bind(binding.exchange, binding.topic, function (queue)
            {
                if (queue)
                {
                    log.info("Bound queue '" + queueName + "' to exchange '" + binding.exchange + "'");
                    ...
                    {                       
                        ...
                        q.subscribe(sub.options, function (message)
                        {
                            var acked = false;
                            var ack = function ()
                            {
                        ...
                        q.on('basicQosOk', function (a, b, c, d)
                        {
                            log.info("basicQosOk", arguments);
                        });
                        //XXX TODO I guess we're supposed to wait for the basicQosOk event here??
                        return me.ok(callback);
                    }
                    else
                        return me.ok(callback);
