...
RabbitMQ.prototype.__bindQueue = function (me, queueName, callback)
{
    ...
    if (queueInfo == null)
        return me.error(callback, "Can't process bindings for undeclared queue '" + queueName + "'");
    ...
    if (bindings != null && bindings instanceof Array && bindings.length > 0 && me.checkRole(bindings[0].roles))
    {
        ...
        queueInfo.queue.bind(exchangeName, binding.topic, function (queue)
        {
            if (queue)
            {
                ...
                if (sub && me.checkRole(sub.roles) && sub.processed !== true)
                {
                    sub.processed = true;
...
RabbitMQ.prototype.__declareQueue = function (me, queueName, queueConfig, callback)
{
    ...
    var q = me.queues[queueName].queue = me.connection.queue(declareQueueName, queueConfig, function (queue, dunno1, dunno2) // OR: q.on('queueDeclareOk', function(queue)
    {
        if (queue == null)
...
RabbitMQ.prototype.__subscribe = function (me, callback)
{
    ...
    function next()
    {
        ...
        var prefix = "(" + j.queueName + "/" + sub.function + ") ";

        log.info(prefix + "Subscribing to queue '" + queueName + "'..");
        q.subscribe(sub.options, function (message, headers, deliveryInfo, messageObject)
        {
            ...
            {
                log.error(prefix + "Got message, but no callback has been configured!");
                ack();
            }
        });
        ...
        {
            log.debug(prefix + "basicQosOk", arguments);
            setImmediate(next);
        });
    }
